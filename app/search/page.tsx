...
export default function SearchPage() {
  ...
  const loadProducts = useCallback(async () => {
    ...
    const sellerIds = [
      ...new Set((productData || []).map((product) => product.seller_id)),
    ];
    ...
    if (sellerIds.length > 0) {
      ...
      sellerMap = Object.fromEntries(
        (sellers || []).map((seller) => [seller.id, seller.username]),
      );
    }

    const cleanProducts: Product[] = (productData || []).map((product) => ({
      id: product.id,
      title: product.title,
      brand: product.brand,
      ...
      seller_username: sellerMap[product.seller_id] || "vendedor",
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((product) => {
      ...
      const matchesProvince = !province || product.province === province;

      const minimum = minPrice === "" ? null : Number(minPrice);

      const maximum = maxPrice === "" ? null : Number(maxPrice);

      const matchesMin = minimum === null || Number(product.price) >= minimum;

      const matchesMax = maximum === null || Number(product.price) <= maximum;
      ...
      <div className="mx-auto max-w-md">
          ...
          <div className="flex items-center gap-3 px-4 py-4">
            ...
            <button
              ...
              className={`relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${
                hasFilters ? "bg-black text-white" : "bg-zinc-100"
              }`}
            >
              <Filter size={18} />
        ...
        {!loading && !error && filteredProducts.length === 0 && (
          <div className="px-5 py-16 text-center">
            <Search size={32} className="mx-auto text-zinc-300" />

            <h2 className="mt-4 font-bold">No encontramos resultados</h2>

            <p className="mt-2 text-sm text-zinc-500">
              Intenta buscar otra marca, artículo o vendedor.
            </p>
