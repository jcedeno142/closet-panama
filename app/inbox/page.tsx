...
type OfferEvent = {
  ...
  offer_id: string;
  actor_id: string;
  event_type: "offer" | "counter" | "accepted" | "declined" | "cancelled";
  amount: number | null;
  created_at: string;
};
...
export default function InboxPage() {
  ...
  const [inboxTab, setInboxTab] = useState<"messages" | "offers">("messages");

  const [receivedOffers, setReceivedOffers] = useState<OfferCard[]>([]);

  const [sentOffers, setSentOffers] = useState<OfferCard[]>([]);

  const [loading, setLoading] = useState(true);
  const [message, setMessage] = useState("");

  const [counterOffer, setCounterOffer] = useState<OfferCard | null>(null);
  ...
  const loadInbox = useCallback(async () => {
    ...
    const allOffers = [...(receivedData || []), ...(sentData || [])] as Offer[];

    const offerIds = [...new Set(allOffers.map((offer) => offer.id))];

    const productIds = [...new Set(allOffers.map((offer) => offer.product_id))];

    const profileIds = [
      ...new Set(
        allOffers.flatMap((offer) => [offer.buyer_id, offer.seller_id]),
      ),
    ];
    ...
    if (offerIds.length > 0) {
      ...
      for (const event of eventData || []) {
        ...
        eventMap[event.offer_id].push(event as OfferEvent);
      }
    }

    const receivedCards: OfferCard[] = (receivedData || []).map((offer) => ({
      ...offer,
      product: productMap[offer.product_id],
      buyer: profileMap[offer.buyer_id],
      seller: profileMap[offer.seller_id],
      events: eventMap[offer.id] || [],
    }));

    const sentCards: OfferCard[] = (sentData || []).map((offer) => ({
      ...offer,
      product: productMap[offer.product_id],
      buyer: profileMap[offer.buyer_id],
      seller: profileMap[offer.seller_id],
  ...
  function getLatestMoneyEvent(offer: OfferCard) {
    return [...offer.events]
      .reverse()
      .find(
        (event) =>
          event.event_type === "offer" || event.event_type === "counter",
      );
  }

  function canCurrentUserRespond(offer: OfferCard) {
    if (
    ...
    const latest = getLatestMoneyEvent(offer);

    if (!latest) return false;

    // Person who made the last monetary
    // offer must wait for the other person.
    return latest.actor_id !== currentUserId;
  }

  function openCounterModal(offer: OfferCard) {
    const latest = getLatestMoneyEvent(offer);
  ...
  async function submitCounterOffer(event: React.FormEvent) {
    ...
    if (!amount || amount <= 0) {
      setMessage("Ingresa una contraoferta válida.");
      return;
    }

    const latest = getLatestMoneyEvent(counterOffer);

    if (!latest?.amount) {
      setMessage("No pudimos encontrar la última oferta.");
      return;
    }

    // Prevent offering full listing price
    if (counterOffer.product && amount >= Number(counterOffer.product.price)) {
      setMessage("La contraoferta debe ser menor que el precio publicado.");
      return;
    }

    if (amount === Number(latest.amount)) {
      setMessage("La contraoferta debe ser diferente a la oferta actual.");
      return;
    }
  ...
  async function acceptCurrentOffer(offer: OfferCard) {
    const latest = getLatestMoneyEvent(offer);

    if (!latest?.amount) {
      setMessage("No pudimos encontrar la oferta actual.");

      return;
    }

    const finalAmount = Number(latest.amount);
  ...
  async function declineOffer(offer: OfferCard) {
    const { error: eventError } = await supabase.from("offer_events").insert({
      offer_id: offer.id,
      actor_id: currentUserId,
      event_type: "declined",
  ...
  return (
    <main className="min-h-screen bg-white pb-10 text-black">
      <div className="mx-auto max-w-md">
        ...
        <header className="sticky top-0 z-40 border-b border-zinc-100 bg-white">
          <div className="flex items-center px-4 py-4">
            ...
            <div className="ml-4">
              ...
              <p className="text-xs text-zinc-400">Mensajes y ofertas</p>
            </div>
          </div>
            ...
            <div className="flex bg-zinc-50">
              <button
                ...
                className={`flex-1 py-3 text-xs font-bold ${
                  activeTab === "received" ? "text-black" : "text-zinc-400"
                }`}
              >
                Recibidas
                {receivedOffers.length > 0 && (
                  <span className="ml-2 rounded-full bg-white px-2 py-0.5 text-[10px]">
              ...
              <button
                ...
                className={`flex-1 py-3 text-xs font-bold ${
                  activeTab === "sent" ? "text-black" : "text-zinc-400"
                }`}
              >
                Enviadas
                {sentOffers.length > 0 && (
        ...
        {inboxTab === "messages" && <MessagesInbox />}
          ...
          <>
            ...
            {!loading && offers.length > 0 && (
              <section className="divide-y divide-zinc-100">
                {offers.map((offer) => {
                  const cover = offer.product?.product_images?.[0]?.image_url;

                  const otherUser =
                    activeTab === "received" ? offer.buyer : offer.seller;

                  const latest = getLatestMoneyEvent(offer);

                  const canRespond = canCurrentUserRespond(offer);

                  return (
                    <article key={offer.id} className="px-4 py-5">
                      {/* PRODUCT */}
                      ...
                      {canRespond && (
                        <div className="mt-4 grid grid-cols-3 gap-2">
                          <button
                            type="button"
                            onClick={() => acceptCurrentOffer(offer)}
        ...
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/50 sm:items-center">
          <div className="w-full max-w-md rounded-t-3xl bg-white p-5 sm:rounded-3xl">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-xl font-bold">Contraofertar</h2>

                <p className="mt-1 text-sm text-zinc-500">
                  Última oferta: $
                  {Number(
                    getLatestMoneyEvent(counterOffer)?.amount || 0,
                  ).toFixed(2)}
                </p>
              </div>
            ...
            <form onSubmit={submitCounterOffer} className="mt-6">
              <label className="text-sm font-bold">Nueva oferta</label>
              ...
              >
                {submittingCounter ? "Enviando..." : "Enviar contraoferta"}
              </button>
            </form>
          </div>
        </div>
...
function StatusBadge({ status }: { status: string }) {
  ...
  const labels: Record<string, string> = {
    pending: "Negociación abierta",

    countered: "Negociación abierta",
